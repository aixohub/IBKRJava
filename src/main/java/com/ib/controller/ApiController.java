...
public class ApiController implements EWrapper {
  ...
  @Override
  public void updatePortfolio(Contract contract, Decimal positionIn, double marketPrice,
      double marketValue, double averageCost, double unrealizedPNL, double realizedPNL,
      String account) {
    contract.exchange(contract.primaryExch());

    Position position = new Position(contract, account, positionIn, marketPrice, marketValue,
  ...
  public void reqTopMktData(Contract contract, String genericTickList, boolean snapshot,
      boolean regulatorySnapshot, ITopMktDataHandler handler) {
    if (!checkConnection()) {
      return;
    }
    ...
    m_client.reqMktData(reqId, contract, genericTickList, snapshot, regulatorySnapshot,
        Collections.emptyList());
    sendEOM();
  }

  public void reqOptionMktData(Contract contract, String genericTickList, boolean snapshot,
      boolean regulatorySnapshot, IOptHandler handler) {
    if (!checkConnection()) {
      return;
    }
  ...
  public void reqEfpMktData(Contract contract, String genericTickList, boolean snapshot,
      boolean regulatorySnapshot, IEfpHandler handler) {
    if (!checkConnection()) {
      return;
    }
  ...
  @Override
  public void updateMktDepthL2(int reqId, int position, String marketMaker, int operation, int side,
  ...
  // ---------------------------------------- Option computations ----------------------------------------
  public void reqOptionVolatility(Contract c, double optPrice, double underPrice,
      IOptHandler handler) {
    if (!checkConnection()) {
      return;
    }
  ...
  @Override
  public void tickOptionComputation(int reqId, int tickType, int tickAttrib, double impliedVol,
  ...
  public void updateGroups(List<Group> groups) {
    ...
    m_client.replaceFA(REPLACE_FA_GROUPS_REQ_ID, FADataType.GROUPS.ordinal(),
        AdvisorUtil.getGroupsXml(groups));
    sendEOM();
  }
  ...
  public void updateProfiles(List<Profile> profiles) {
    ...
    m_client.replaceFA(REPLACE_FA_PROFILES_REQ_ID, FADataType.PROFILES.ordinal(),
        AdvisorUtil.getProfilesXml(profiles));
    sendEOM();
  }
  ...
  public void cancelOrder(int orderId, String manualOrderCancelTime,
      final IOrderCancelHandler orderCancelHandler) {
    if (!checkConnection()) {
      return;
    }
  ...
  public void exerciseOption(String account, Contract contract, ExerciseType type, int quantity,
      boolean override) {
    if (!checkConnection()) {
      return;
    }

    m_client.exerciseOptions(m_reqId++, contract, type.ordinal(), quantity, account,
        override ? 1 : 0);
    sendEOM();
  }
  ...
  public void reqScannerSubscription(ScannerSubscription sub, List<TagValue> filterOptions,
      IScannerHandler handler) {
    if (!checkConnection()) {
      return;
    }
   ...
   */
  public void reqHistoricalData(Contract contract, String endDateTime, int duration,
      DurationUnit durationUnit, BarSize barSize, WhatToShow whatToShow, boolean rthOnly,
      boolean keepUpToDate, IHistoricalDataHandler handler) {
    if (!checkConnection()) {
      return;
    }
    ...
    m_client.reqHistoricalData(reqId, contract, endDateTime, durationStr, barSize.toString(),
        whatToShow.toString(), rthOnly ? 1 : 0, 1, keepUpToDate, Collections.emptyList());
    sendEOM();
  }
  ...
  public void historicalData(int reqId, com.ib.client.Bar bar) {
    ...
    if (handler != null) {
      if (bar.time().startsWith("finished")) {
        ...
        Bar bar2 = new Bar(bar.time(), bar.high(), bar.low(), bar.open(), bar.close(), bar.wap(),
  ...
  public void reqRealTimeBars(Contract contract, WhatToShow whatToShow, boolean rthOnly,
    ...
    m_client.reqRealTimeBars(reqId, contract, 0, whatToShow.toString(), rthOnly,
        realTimeBarsOptions);
    sendEOM();
  }
  ...
  protected boolean checkConnection() {
    if (!isConnected()) {
      error(EClientErrors.NO_VALID_ID, EClientErrors.NOT_CONNECTED.code(),
          EClientErrors.NOT_CONNECTED.msg(), null);
      return false;
    }
  ...
  public void reqAccountUpdatesMulti(String account, String modelCode, boolean ledgerAndNLV,
      IAccountUpdateMultiHandler handler) {
    if (!checkConnection()) {
      return;
    }
  ...
  public void reqSecDefOptParams(String underlyingSymbol,
      String futFopExchange, /*String currency,*/ String underlyingSecType, int underlyingConId,
      ISecDefOptParamsReqHandler handler) {
    if (!checkConnection()) {
      return;
    }
    ...
    m_client.reqSecDefOptParams(reqId, underlyingSymbol, futFopExchange, /*currency,*/
  ...
  public void securityDefinitionOptionalParameter(int reqId, String exchange, int underlyingConId,
    ...
    if (handler != null) {
      handler.securityDefinitionOptionalParameter(exchange, underlyingConId, tradingClass,
  ...
  public void reqHistoricalNews(int conId, String providerCodes, String startDateTime,
      String endDateTime, int totalResults, IHistoricalNewsHandler handler) {
    if (!checkConnection()) {
      return;
    }
    ...
    m_client.reqHistoricalNews(requestId, conId, providerCodes, startDateTime, endDateTime,
  ...
  public void reqHistogramData(Contract contract, int duration, DurationUnit durationUnit,
      boolean rthOnly, IHistogramDataHandler handler) {
    if (!checkConnection()) {
      return;
    }
  ...
  public void rerouteMktDataReq(int reqId, int conId, String exchange) {
    show("Re-route market data request. ReqId: " + reqId + ", ConId: " + conId + ", Exchange: "
        + exchange);
  }

  @Override
  public void rerouteMktDepthReq(int reqId, int conId, String exchange) {
    show("Re-route market depth request. ReqId: " + reqId + ", ConId: " + conId + ", Exchange: "
        + exchange);
  }
  ...
  public void reqHistoricalTicks(Contract contract, String startDateTime,
      ...
      IHistoricalTickHandler handler) {
    if (!checkConnection()) {
      return;
    }
    ...
    m_client.reqHistoricalTicks(reqId, contract, startDateTime, endDateTime, numberOfTicks,
        whatToShow, useRth, ignoreSize, Collections.emptyList());
  }
  ...
  public void tickByTickAllLast(int reqId, int tickType, long time, double price, Decimal size,
    ...
    if (handler != null) {
      handler.tickByTickAllLast(reqId, tickType, time, price, size, tickAttribLast, exchange,
          specialConditions);
    }
  ...
  public void orderBound(long orderId, int apiClientId, int apiOrderId) {
    show("Order bound. OrderId: " + orderId + ", apiClientId: " + apiClientId + ", apiOrderId: "
        + apiOrderId);
  }
  ...
  public void reqHistoricalSchedule(Contract contract, String endDateTime, int duration,
      DurationUnit durationUnit, boolean rthOnly, IHistoricalScheduleHandler handler) {
    if (!checkConnection()) {
      return;
    }
    ...
    String durationStr = duration + " " + durationUnit.toString().charAt(0);
    m_client.reqHistoricalData(reqId, contract, endDateTime, durationStr, BarSize._1_day.toString(),
        WhatToShow.SCHEDULE.name(), rthOnly ? 1 : 0, 2, false, Collections.emptyList());
