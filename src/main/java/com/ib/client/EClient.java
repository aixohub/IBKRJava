...
public abstract class EClient {
  ...
  protected void sendConnectRequest() throws IOException {
    ...
    if (!m_useV100Plus || m_connectOptions == null) {
      send(
          CLIENT_VERSION); // Do not add length prefix here, because Server does not know Client's version yet
    } else {
  ...
  public synchronized void reqMktData(int tickerId, Contract contract,
      String genericTickList, boolean snapshot, boolean regulatorySnapshot,
  ...
  public synchronized void reqRealTimeBars(int tickerId, Contract contract, int barSize,
      String whatToShow, boolean useRTH, List<TagValue> realTimeBarsOptions) {
    // not connected?
    if (!isConnected()) {
      notConnected();
      return;
  ...
  public synchronized void reqMktDepth(int tickerId, Contract contract, int numRows,
      boolean isSmartDepth, List<TagValue> mktDepthOptions) {
    // not connected?
    if (!isConnected()) {
      notConnected();
      return;
    ...
    if (m_serverVersion < MIN_SERVER_VER_MKT_DEPTH_PRIM_EXCHANGE && !IsEmpty(
  ...
  public synchronized void placeOrder(int id, Contract contract, Order order) {
    ...
    if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && SecType.BAG.name()
        .equalsIgnoreCase(contract.getSecType())) {
    ...
    if (m_serverVersion < MIN_SERVER_VER_SCALE_TABLE) {
      if (!IsEmpty(order.scaleTable()) || !IsEmpty(order.activeStartTime()) || !IsEmpty(
    ...
    try {
      ...
      if (m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS && SecType.BAG.name()
          .equalsIgnoreCase(contract.getSecType())) {
      ...
      if (m_serverVersion >= 26) { // Volatility orders
        ...
        if (m_serverVersion < 28) {
          ...
          if (m_serverVersion >= MIN_SERVER_VER_DELTA_NEUTRAL_CONID && !IsEmpty(
          ...
          if (m_serverVersion >= MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE && !IsEmpty(
              order.getDeltaNeutralOrderType())) {
        ...
        if (m_serverVersion == 26) {
          // Volatility orders had specific watermark price attribs in server version 26
          double lower =
              order.getOrderType().equals("VOL") ? order.stockRangeLower() : Double.MAX_VALUE;
          double upper =
              order.getOrderType().equals("VOL") ? order.stockRangeUpper() : Double.MAX_VALUE;
          b.sendMax(lower);
          b.sendMax(upper);
        }
  ...
  public synchronized void reqSecDefOptParams(int reqId, String underlyingSymbol,
  ...
  public synchronized void reqAccountUpdatesMulti(int reqId, String account, String modelCode,
      boolean ledgerAndNLV) {
    // not connected?
    if (!isConnected()) {
      notConnected();
      return;
  ...
  public synchronized void verifyAndAuthRequest(String apiName, String apiVersion,
      String opaqueIsvKey) {
    // not connected?
    if (!isConnected()) {
      notConnected();
  ...
  public synchronized void subscribeToGroupEvents(int reqId, int groupId) {
    ...
    try {
      ...
      error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_SUBSCRIBETOGROUPEVENTS,
          e.toString());
    }
  }
  ...
  public synchronized void unsubscribeFromGroupEvents(int reqId) {
    ...
    try {
      ...
      error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_UNSUBSCRIBEFROMGROUPEVENTS,
          e.toString());
    }
  }
  ...
  public synchronized void reqTickByTickData(int reqId, Contract contract, String tickType,
      int numberOfTicks, boolean ignoreSize) {
    // not connected?
    if (!isConnected()) {
      notConnected();
      return;
  ...
  public synchronized void reqWshEventData(int reqId, WshEventData wshEventData) {
    ...
    if (m_serverVersion < MIN_SERVER_VER_WSH_EVENT_DATA_FILTERS) {
      if (!IsEmpty(wshEventData.filter()) || wshEventData.fillWatchlist()
          || wshEventData.fillPortfolio() || wshEventData.fillCompetitors()) {
        error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS,
    ...
    if (m_serverVersion < MIN_SERVER_VER_WSH_EVENT_DATA_FILTERS_DATE) {
      if (!IsEmpty(wshEventData.startDate()) || !IsEmpty(wshEventData.endDate())
          || wshEventData.totalLimit() != Integer.MAX_VALUE) {
        error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS,
