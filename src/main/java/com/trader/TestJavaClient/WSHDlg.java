...
public class WSHDlg extends JDialogBox {
  ...
  private final JCheckBox m_fillWatchlistCheckbox = new JCheckBox("Fill Watchlist", false);
  private final JCheckBox m_fillPortfolioCheckbox = new JCheckBox("Fill Portfolio", false);
  private final JCheckBox m_fillCompetitorsCheckbox = new JCheckBox("Fill Competitors", false);
  private final JTextField m_startDateField = new JTextField();
  private final JTextField m_endDateField = new JTextField();
  private final JTextField m_totalLimitField = new JTextField();
  ...
  protected void onOk() {
    try {
      ...
      if (m_isWshEventDlg) {
        int conId = m_conIdField.getText().length() > 0 ? Integer.parseInt(m_conIdField.getText())
            : Integer.MAX_VALUE;
        int totalLimit =
            m_totalLimitField.getText().length() > 0 ? Integer.parseInt(m_totalLimitField.getText())
                : Integer.MAX_VALUE;
