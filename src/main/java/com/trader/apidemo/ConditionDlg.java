...
public class ConditionDlg extends JDialog implements ChangeListener, ActionListener {
  ...
  public void stateChanged(ChangeEvent e) {
    ...
    if (m_rbMargin.isSelected()) {
      m_conditionSubPanel = new MarginConditionPanel(
          (MarginCondition) instantiateCondition(MarginCondition.conditionType));
    }

    if (m_rbPercent.isSelected()) {
      m_conditionSubPanel = new PercentConditionPanel(
          (PercentChangeCondition) instantiateCondition(PercentChangeCondition.conditionType),
          m_lookuper);
    }

    if (m_rbPrice.isSelected()) {
      m_conditionSubPanel = new PriceConditionPanel(
          (PriceCondition) instantiateCondition(PriceCondition.conditionType), m_lookuper);
    }

    if (m_rbTime.isSelected()) {
      m_conditionSubPanel = new TimeConditionPanel(
          (TimeCondition) instantiateCondition(TimeCondition.conditionType));
    }

    if (m_rbTrade.isSelected()) {
      m_conditionSubPanel = new TradeConditionPanel(
          (ExecutionCondition) instantiateCondition(ExecutionCondition.conditionType));
    }

    if (m_rbVolume.isSelected()) {
      m_conditionSubPanel = new VolumeConditionPanel(
          (VolumeCondition) instantiateCondition(VolumeCondition.conditionType), m_lookuper);
    }
