...
public class ContractSearchDlg extends JDialog {
  ...
  ContractSearchDlg(int conId, String exchange, ContractLookuper lookuper) {
    ...
    m_contracts.setCellRenderer(
        (list, value, index, isSelected, cellHasFocus) -> onGetListCellRenderer(value, isSelected));
  ...
  String refContract() {
    ...
    return selectedContract().symbol() + " " + selectedContract().exchange() + " "
        + selectedContract().secType() + " " + selectedContract().currency();
  }
