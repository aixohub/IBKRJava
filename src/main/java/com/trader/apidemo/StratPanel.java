...
import com.trader.apidemo.util.VerticalPanel.BorderPanel;
...
public class StratPanel extends StackPanel implements IHistoricalDataHandler, IRealTimeBarHandler {
  ...
  private final Map<Long, Bar> m_map = new TreeMap<>();
  ...
  public void realtimeBar(Bar bar) {
    if (!m_req) {
      ...
      ApiDemo.INSTANCE.controller()
          .reqHistoricalData(m_contract, date, duration, queryLength.m_unit, barSize,
              WhatToShow.TRADES, false, false, this);
      m_req = true;
    }
    addBar(bar);
    m_chart.repaint();
