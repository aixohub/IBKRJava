...
import com.trader.apidemo.util.Util;
import com.trader.apidemo.util.VerticalPanel;
import com.trader.apidemo.util.VerticalPanel.HorzPanel;
...
class TicketDlg extends JDialog {
  ...
  private void onCancelOrder() {
    ...
    if (m_order != null) {
      ApiDemo.INSTANCE.controller()
          .cancelOrder(m_order.orderId(), manualOrderCancelTime, new IOrderCancelHandler() {
            @Override
  ...
  private void displayMargin(OrderState orderState) {
    String str = String.format(
        "Current:%nEquity with loan: %s%nInitial margin: %s%nMaintenance margin: %s%n%n"
        ...
        orderState.equityWithLoanBefore() != null ? fmt(
            Double.parseDouble(orderState.equityWithLoanBefore())) : "",
        orderState.initMarginBefore() != null ? fmt(
            Double.parseDouble(orderState.initMarginBefore())) : "",
        orderState.maintMarginBefore() != null ? fmt(
            Double.parseDouble(orderState.maintMarginBefore())) : "",
        orderState.equityWithLoanChange() != null ? fmt(
            Double.parseDouble(orderState.equityWithLoanChange())) : "",
        orderState.initMarginChange() != null ? fmt(
            Double.parseDouble(orderState.initMarginChange())) : "",
        orderState.maintMarginChange() != null ? fmt(
            Double.parseDouble(orderState.maintMarginChange())) : "",
    ...
    JOptionPane.showMessageDialog(this, str, "Margin Requirements",
  ...
  class OrderPanel extends VerticalPanel {
    ...
    final TCombo<String> m_account = new TCombo<>(
        ApiDemo.INSTANCE.accountList().toArray(new String[0]));
    ...
    OrderPanel() {
      ...
      m_account.setSelectedItem(
          m_order.account() != null ? m_order.account() : ApiDemo.INSTANCE.accountList().get(0));
      ...
      m_usePriceMgmtAlgo.setSelectedIndex(
          m_order.usePriceMgmtAlgo() == null ? 0 : m_order.usePriceMgmtAlgo() ? 2 : 1);
  ...
  class VolatilityTicketPanel extends VerticalPanel {
    ...
    final TCombo<ReferencePriceType> m_referencePriceType = new TCombo<>(
        ReferencePriceType.values());
  ...
  /**
   * This panel edits all ComboParam values except for Non-Guaranteed. That one goes on main panel
   * because it applies to all combo orders.
   */
  ...
  class ScalePanel extends VerticalPanel {
    ...
    ScalePanel() {
      ...
      add(Box.createVerticalStrut(10));
      add("Increase price by", m_priceAdjustValue, new JLabel("every"), m_priceAdjustInterval,
          new JLabel("seconds"));
      add(Box.createVerticalStrut(10));
