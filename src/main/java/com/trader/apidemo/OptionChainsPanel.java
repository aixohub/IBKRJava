...
class OptionChainsPanel extends JPanel {
  ...
  private final UpperField m_lastTradeDateOrContractMonth = new UpperField();
  private final UpperField m_exchange = new UpperField();
  private final UpperField m_currency = new UpperField();
  private final JCheckBox m_snapshot = new JCheckBox();
  private final TCombo<String> m_marketDataType = new TCombo<>(MarketDataType.getFields());
  ...
  private class ChainPanel extends NewTabPanel implements IContractDetailsHandler, ActionListener {
    ...
    ChainPanel() {
      ...
      ApiDemo.INSTANCE.controller()
          .reqTopMktData(m_underContract, "", false, false, m_stockListener);
    }
    ...
    private class ChainModel extends AbstractTableModel {
      ...
      Comparator<ChainRow> c = (o1, o2) -> {
        int rc = o1.m_c.lastTradeDateOrContractMonth()
            .compareTo(o2.m_c.lastTradeDateOrContractMonth());
        if (rc == 0) {
      ...
      void addRow(Contract contract, boolean snapshot) {
        ...
        if (snapshot) {
          sleep(11); // try to avoid pacing violation at TWS
        }
      }
      ...
      private class ChainRow extends TopMktDataAdapter implements IOptHandler {
        ...
        @Override
        public void tickOptionComputation(TickType tickType, int tickAttrib, double impVol,
