...
import com.trader.TestJavaClient.SmartComboRoutingParamsDlg;
...
import com.trader.apidemo.util.UpperField;
import com.trader.apidemo.util.VerticalPanel;
import com.trader.apidemo.util.VerticalPanel.StackPanel;
...
class MarketDataPanel extends JPanel {
  ...
  private final Set<String> m_bboExchanges = new HashSet<>();
  private final RequestSmartComponentsPanel m_smartComponentsPanel = new RequestSmartComponentsPanel();
  ...
  static class SymbolSamplesPanel extends NewTabPanel implements ISymbolSamplesHandler {
    ...
    static class SymbolSamplesRow {
      ...
      SymbolSamplesRow(int conId, String symbol, String secType, String primaryExch,
          String currency, String derivativeSecTypes, String description, String issuerId) {
        update(conId, symbol, secType, primaryExch, currency, derivativeSecTypes, description,
            issuerId);
      }

      void update(int conId, String symbol, String secType, String primaryExch, String currency,
          String derivativeSecTypes, String description, String issuerId) {
        m_conId = conId;
        m_symbol = symbol;
        m_secType = secType;
  ...
  private static class DeepResultsPanel extends NewTabPanel implements IDeepMktDataHandler {
    ...
    @Override
    public void updateMktDepth(int pos, String mm, DeepType operation, DeepSide side, double price,
        Decimal size) {
    ...
    static class DeepModel extends AbstractTableModel {
      ...
      public void updateMktDepth(int pos, String mm, DeepType operation, double price,
          Decimal size) {
        switch (operation) {
          case INSERT:
  ...
  static class BarResultsPanel extends NewTabPanel implements IHistoricalDataHandler,
      IRealTimeBarHandler {
  ...
  private class WSHCalendarRequestPanel extends JPanel {
    ...
    private final JCheckBox m_fillWatchlistCheckbox = new JCheckBox();
    private final JCheckBox m_fillPortfolioCheckbox = new JCheckBox();
    private final JCheckBox m_fillCompetitorsCheckbox = new JCheckBox();
  ...
  private class RequestSmartComponentsPanel extends JPanel {
    ...
    RequestSmartComponentsPanel() {
      ...
      VerticalPanel paramsPanel = new VerticalPanel();
      paramsPanel.add("BBO Exchange", m_BBOExchList, Box.createHorizontalStrut(10),
          requestSmartComponentsButton);
      setLayout(new BorderLayout());
      add(paramsPanel, BorderLayout.NORTH);
    }
    ...
    void onRequestSmartComponents() {
      ...
      ApiDemo.INSTANCE.controller()
          .reqSmartComponents(m_BBOExchList.getSelectedItem(), m_smartComponentsResPanel);
    }
  }
  ...
  private class RequestMatchingSymbolsPanel extends JPanel {
    ...
    RequestMatchingSymbolsPanel() {
      ...
      VerticalPanel paramsPanel = new VerticalPanel();
      paramsPanel.add("Pattern", m_pattern, Box.createHorizontalStrut(10),
          requestMatchingSymbolsButton);
      setLayout(new BorderLayout());
      add(paramsPanel, BorderLayout.NORTH);
    }
    ...
    void onRequestMatchingSymbols() {
      ...
      m_resultsPanel.addTab("Symbol Samples " + m_pattern.getText(), symbolSamplesPanel, true,
          true);
  ...
  private class DeepRequestPanel extends JPanel {
    ...
    void onDeep() {
      ...
      m_resultsPanel.addTab((isSmartDepth ? "SMART" : "Market") + " depth " + m_contract.symbol(),
          resultPanel, true, true);
      ApiDemo.INSTANCE.controller()
          .reqDeepMktData(m_contract, numOfRows, isSmartDepth, resultPanel);
    }
  }
  ...
  private class HistRequestPanel extends JPanel {
    ...
    protected void onHistoricalSchedule() {
      ...
      ApiDemo.INSTANCE.controller()
          .reqHistoricalSchedule(m_contract, m_end.getText(), m_duration.getInt(),
    ...
    protected void onHistoricalTick() {
      ...
      ApiDemo.INSTANCE.controller()
          .reqHistoricalTicks(m_contract, m_begin.getText(), m_end.getText(),
              m_nTicks.getInt(), m_whatToShow.getSelectedItem().name(),
              m_rthOnly.isSelected() ? 1 : 0,
  ...
  private class RealtimeRequestPanel extends JPanel {
    ...
    void onHts() {
      ...
      ApiDemo.INSTANCE.controller()
          .reqHeadTimestamp(m_contract, m_whatToShow.getSelectedItem(), m_rthOnly.isSelected(),
              panel);
    ...
    void onRealTime() {
      ...
      ApiDemo.INSTANCE.controller()
          .reqRealTimeBars(m_contract, m_whatToShow.getSelectedItem(), m_rthOnly.isSelected(),
              panel);
  ...
  private class ScannerRequestPanel extends JPanel {
    ...
    protected void onFilterOptions() {
      SmartComboRoutingParamsDlg smartComboRoutingParamsDlg = new SmartComboRoutingParamsDlg(
          "Scanner Subscription Filter Options", m_filterOptions,
  ...
  class PnLPanel extends JPanel {
    ...
    protected void onReqPnLSingle() {
      ...
      IPnLSingleHandler handler = (reqId, pos, dailyPnL, unrealizedPnL, realizedPnL, value) ->
          SwingUtilities.invokeLater(
              () -> pnlSingleModel.addRow(pos, dailyPnL, unrealizedPnL, realizedPnL, value));
  ...
  private class TickByTickRequestPanel extends JPanel {
    ...
    protected void onReqTickByTickData() {
      ...
      TickByTickResultsPanel panel = new TickByTickResultsPanel(
          TickByTickType.valueOf(m_tickType.getSelectedItem().name()));

      ApiDemo.INSTANCE.controller()
          .reqTickByTickData(m_contract, m_tickType.getSelectedItem().name(),
              m_numberOfTicks.getInt(), m_ignoreSize.isSelected(), panel);
      m_resultsPanel.addTab("Tick-By-Tick " + (m_numberOfTicks.getInt() > 0 ? "Hist + " : "")
          + m_tickType.getSelectedItem().name() + " " + m_contract.symbol(), panel, true, true);
    }
  }
}
