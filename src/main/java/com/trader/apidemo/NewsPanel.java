...
class NewsPanel extends JPanel {
  ...
  private final NewsArticleRequestPanel m_newsArticleRequestPanel = new NewsArticleRequestPanel();
  ...
  private static class RequestPanel extends JPanel {
    ...
    RequestPanel() {
      ...
      choosePathDialogButton.addActionListener(e -> m_path.setText(
          chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION ? chooser.getSelectedFile()
              .getPath() : m_path.getText()));
      ...
      if (articleType == 0) {
        ...
        if (articleType == 1) {
          try {
            ...
            m_text.setText("Binary/pdf article was not saved to " + m_path + " due to error: "
                + ex.getMessage());
          }
        }
  ...
  class NewsTicksRequestPanel extends JPanel {
    ...
    private final TCombo<SecType> m_secType = new TCombo<>(SecType.values());
    private final UpperField m_exchange = new UpperField();
    private final UpperField m_primExchange = new UpperField();
    private final UpperField m_currency = new UpperField();
  ...
  class NewsTicksResultsPanel extends NewTabPanel implements ITickNewsHandler {
    ...
    @Override
    public void tickNews(long timeStamp, String providerCode, String articleId, String headline,
        String extraData) {
      NewsTickRow newsTickRow = new NewsTickRow(timeStamp, providerCode, articleId, headline,
          extraData);
    ...
    class NewsTickRow {
      ...
      NewsTickRow(long timeStamp, String providerCode, String articleId, String headline,
          String extraData) {
        update(timeStamp, providerCode, articleId, headline, extraData);
      }

      void update(long timeStamp, String providerCode, String articleId, String headline,
          String extraData) {
  ...
  class HistoricalNewsResultsPanel extends NewTabPanel implements IHistoricalNewsHandler {
    ...
    HistoricalNewsResultsPanel() {
      ...
      table.getSelectionModel().addListSelectionListener(event -> {
        if (!event.getValueIsAdjusting() && table.getSelectedRow() != -1) {
          ...
          if (historicalNewsRow.m_providerCode.length() > 0
              && historicalNewsRow.m_articleId.length() > 0) {
            m_requestPanels.select("News Article");
            m_newsArticleRequestPanel.m_requestPanel.setProviderCode(
                historicalNewsRow.m_providerCode);
    ...
    @Override
    public void historicalNews(String time, String providerCode, String articleId,
        String headline) {
      HistoricalNewsRow historicalNewsRow = new HistoricalNewsRow(time, providerCode, articleId,
          headline);
