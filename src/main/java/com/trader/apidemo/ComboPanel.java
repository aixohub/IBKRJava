...
import com.trader.apidemo.OrdersPanel.OrderRow;
import com.trader.apidemo.OrdersPanel.OrdersModel;
import com.trader.apidemo.TopModel.TopRow;
import com.trader.apidemo.util.HtmlButton;
import com.trader.apidemo.util.NewTabbedPanel;
import com.trader.apidemo.util.NewTabbedPanel.INewTab;
      ...
      protected void onAdd() {
        ...
        ApiDemo.INSTANCE.controller().reqContractDetails(dn, list -> {
          if (list.size() == 1) {
            ...
            m_dnContract = new DeltaNeutralContract(c.conid(), m_delta.getDouble(),
                m_price.getDouble());
            m_dnText.setText(
                String.format("Delta-neutral: %s Delta: %s  Price: %s", c.textDescription(),
                    m_delta.getText(), m_price.getText()));
          } else {
  ...
  static class EfpPanel extends JPanel {
    ...
    static class EfpModel extends AbstractTableModel {
      ...
      class EfpRow extends TopRow implements IEfpHandler {
        ...
        @Override
        public void tickEFP(int tickType, double basisPoints, String formattedBasisPoints,
