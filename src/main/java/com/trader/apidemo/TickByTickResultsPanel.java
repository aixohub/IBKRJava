...
class TickByTickResultsPanel extends NewTabPanel implements ITickByTickDataHandler {
  ...
  public void tickByTickAllLast(int reqId, int tickType, long time, double price, Decimal size,
    ...
    TickByTick tick = new TickByTick(tickType, time, price, size, tickAttribLast, exchange,
        specialConditions);
  ...
  public void tickByTickHistoricalTickAllLast(int reqId, List<HistoricalTickLast> ticks) {
    for (HistoricalTickLast tick : ticks) {
      TickByTick tickByTick = new TickByTick(2, tick.time(), tick.price(), tick.size(),
  ...
  public void tickByTickHistoricalTickBidAsk(int reqId, List<HistoricalTickBidAsk> ticks) {
    for (HistoricalTickBidAsk tick : ticks) {
      TickByTick tickByTick = new TickByTick(tick.time(), tick.priceBid(), tick.sizeBid(),
          tick.priceAsk(), tick.sizeAsk(), tick.tickAttribBidAsk());
      m_tickByTickRows.add(tickByTick);
    }
