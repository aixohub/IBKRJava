...
public class ConditionsPanel extends OnOKPanel {
  ...
  private final JCheckBox m_ignoreRth = new JCheckBox(
      "Allow condition to be satisfied and activate order outside of regular trading hours");
  ...
  public ConditionsPanel(JDialog parentDlg, Order order, final ContractLookuper lookuper) {
      ...
      public void mouseClicked(MouseEvent e) {
        ...
        if (m_conditions.getSelectedColumn() == 0) {
          ConditionDlg dlg = new ConditionDlg(
              m_order.conditions().get(m_conditions.getSelectedRow()), lookuper);
    ...
    buttons.add(new HtmlButton("Add") {
      ...
      protected void actionPerformed() {
        ConditionDlg dlg = new ConditionDlg(OrderCondition.create(OrderConditionType.Price),
            lookuper);
