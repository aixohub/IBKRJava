...
package com.trader.samples.testbed;
...
import com.trader.samples.testbed.advisor.FAMethodSamples;
import com.trader.samples.testbed.contracts.ContractSamples;
import com.trader.samples.testbed.orders.AvailableAlgoParams;
import com.trader.samples.testbed.orders.OrderSamples;
import com.trader.samples.testbed.scanner.ScannerSubscriptionSamples;
...
public class Testbed {
  ...
  private static void historicalTicks(EClientSocket client) {
    //! [reqhistoricalticks]
    client.reqHistoricalTicks(18001, ContractSamples.USStockAtSmart(),
        "20220808 10:00:00 US/Eastern", null, 10, "TRADES", 1, true, null);
    client.reqHistoricalTicks(18002, ContractSamples.USStockAtSmart(),
        "20220808 10:00:00 US/Eastern", null, 10, "BID_ASK", 1, true, null);
    client.reqHistoricalTicks(18003, ContractSamples.USStockAtSmart(),
        "20220808 10:00:00 US/Eastern", null, 10, "MIDPOINT", 1, true, null);
    //! [reqhistoricalticks]
  }
  ...
  private static void orderOperations(EClientSocket client, int nextOrderId)
    ...
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(),
        OrderSamples.Midprice("BUY", Decimal.ONE, 150));
    //! [place_midprice]
    ...
    client.placeOrder(nextOrderId++, ContractSamples.CryptoContract(),
        OrderSamples.LimitOrder("BUY", Decimal.parse("0.00001234"), 3370));
    //! [crypto_order_submission]

    //! [manual_order_time]
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(),
        OrderSamples.LimitOrderWithManualOrderTime("BUY", Decimal.get(100), 111.11,
            "20220314-13:00:00"));
    //! [manual_order_time]
    ...
    client.placeOrder(nextOrderId++, ContractSamples.IBKRATSContract(),
        OrderSamples.PegBestUpToMidOrder("BUY", Decimal.get(100), 111.11, 100, 200, 0.02, 0.025));
    //! [pegbest_up_to_mid_order_submission]

    //! [pegbest_order_submission]
    client.placeOrder(nextOrderId++, ContractSamples.IBKRATSContract(),
        OrderSamples.PegBestOrder("BUY", Decimal.get(100), 111.11, 100, 200, 0.03));
    //! [pegbest_order_submission]

    //! [pegmid_order_submission]
    client.placeOrder(nextOrderId++, ContractSamples.IBKRATSContract(),
        OrderSamples.PegMidOrder("BUY", Decimal.get(100), 111.11, 100, 0.02, 0.025));
    //! [pegmid_order_submission]
  ...
  private static void tickDataOperations(EClientSocket client) throws InterruptedException {
    ...
    client.reqMktData(1011, ContractSamples.FLYbroadtapeNewsFeed(), "mdoff,292", false, false,
        null);
    //! [reqmktdata_broadtapenews]
  ...
  private static void historicalDataRequests(EClientSocket client) throws InterruptedException {
    ...
    client.reqHistoricalData(4001, ContractSamples.EurGbpFx(), formatted, "1 M", "1 day",
        "MIDPOINT", 1, 1, false, null);
    client.reqHistoricalData(4002, ContractSamples.EuropeanStock(), formatted, "10 D", "1 min",
        "TRADES", 1, 1, false, null);
    client.reqHistoricalData(4003, ContractSamples.USStockAtSmart(), formatted, "1 M", "1 day",
        "SCHEDULE", 1, 1, false, null);
    Thread.sleep(2000);
  ...
  private static void conditionSamples(EClientSocket client, int nextOrderId) {
    ...
    mkt.conditions()
        .add(OrderSamples.PercentageChangeCondition(15.0, 208813720, "SMART", true, true));
  ...
  private static void testAlgoSamples(EClientSocket client, int nextOrderId)
    ...
    AvailableAlgoParams.FillScaleParams(scaleOrder, 2000, 500, true, .02, 189.00, 3600, 2.00, true,
        10, 40);
    ...
    //! [twap]
    AvailableAlgoParams.FillTwapParams(baseOrder, "Marketable", "10:00:00 US/Eastern",
        "11:00:00 US/Eastern", true);
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(), baseOrder);
    //! [twap]
    ...
    //! [vwap]
    AvailableAlgoParams.FillVwapParams(baseOrder, 0.2, "09:00:00 US/Eastern", "16:00:00 US/Eastern",
        true, true, true);
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(), baseOrder);
    //! [vwap]
    ...
    //! [closepx]
    AvailableAlgoParams.FillClosePriceParams(baseOrder, 0.5, "Neutral", "12:00:00 US/Eastern",
        true);
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(), baseOrder);
    //! [closepx]
    ...
    //! [pctvolpx]
    AvailableAlgoParams.FillPriceVariantPctVolParams(baseOrder, 0.1, 0.05, 0.01, 0.2,
        "12:00:00 US/Eastern", "14:00:00 US/Eastern", true);
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(), baseOrder);
    //! [pctvolpx]
    ...
    //! [jeff_vwap_algo]
    AvailableAlgoParams.FillJefferiesVWAPParams(baseOrder, "10:00:00 US/Eastern",
        "16:00:00 US/Eastern", 10, 10, "Exclude_Both", 130, 135, 1, 10, "Patience", false,
        "Midpoint");
    client.placeOrder(nextOrderId++, ContractSamples.JefferiesContract(), baseOrder);
    //! [jeff_vwap_algo]

    //! [csfb_inline_algo]
    AvailableAlgoParams.FillCSFBInlineParams(baseOrder, "10:00:00 US/Eastern",
        "16:00:00 US/Eastern", "Patient", 10, 20, 100, "Default", false, 40, 100, 100, 35);
    client.placeOrder(nextOrderId++, ContractSamples.CSFBContract(), baseOrder);
    //! [csfb_inline_algo]

    //! [qbalgo_strobe_algo]
    AvailableAlgoParams.FillQBAlgoInlineParams(baseOrder, "10:00:00 US/Eastern",
        "16:00:00 US/Eastern", -99, "TWAP", 0.25, true);
  ...
  private static void bracketSample(EClientSocket client, int nextOrderId) {
    ...
    //BRACKET ORDER
    //! [bracketsubmit]
    List<Order> bracket = OrderSamples.BracketOrder(nextOrderId++, "BUY", Decimal.ONE_HUNDRED, 30,
        40, 20);
    for (Order o : bracket) {
  ...
  private static void fundamentals(EClientSocket client) throws InterruptedException {
    ...
    //! [fundamentalexamples]
    client.reqFundamentalData(8002, ContractSamples.USStock(), "ReportSnapshot",
        null); //for company overview
    client.reqFundamentalData(8003, ContractSamples.USStock(), "ReportRatios",
        null); //for financial ratios
    client.reqFundamentalData(8004, ContractSamples.USStock(), "ReportsFinStatements",
        null); //for financial statements
    client.reqFundamentalData(8005, ContractSamples.USStock(), "RESC",
        null); //for analyst estimates
    client.reqFundamentalData(8006, ContractSamples.USStock(), "CalendarReport",
        null); //for company calendar
    //! [fundamentalexamples]
  ...
  private static void marketScanners(EClientSocket client) throws InterruptedException {
    ...
    //! [reqcomplexscanner]
    List<TagValue> AAPLConIDTag = Collections.singletonList(
        new TagValue("underConID", "265598")); // 265598 is the conID for AAPL stock
  ...
  private static void continuousFuturesOperations(EClientSocket client)
    ...
    SimpleDateFormat form = new SimpleDateFormat("yyyyMMdd-HH:mm:ss");
    String formatted = form.format(cal.getTime());
    client.reqHistoricalData(18002, ContractSamples.ContFut(), formatted, "1 Y", "1 month",
        "TRADES", 0, 1, false, null);
    Thread.sleep(10000);
  ...
  private static void whatIfSamples(EClientSocket client, int nextOrderId)
    ...
    //! [whatiforder]
    client.placeOrder(nextOrderId++, ContractSamples.USStockAtSmart(),
        OrderSamples.WhatIfLimitOrder("BUY", Decimal.get(200), 120));
    //! [whatiforder]
  }
